...
bool HttpAgent::UpdateServiceInfo()
{
    ...
    if (cntl.Failed()) {
        LOG(ERROR) << "UpdateServiceInfo(): call ListService failed: " << cntl.ErrorText();
        return false;
    }
    ...
    for (int i = 0; i < response.files_size(); ++i) {
        ...
        if (fd == NULL) {
            LOG(ERROR) << "UpdateServiceInfo(): build file descriptor failed: " << d.name();
            delete _desc_pool;
            _desc_pool = NULL;
            return false;
        }
        LOG(INFO) << "UpdateServiceInfo(): add file [" << d.name() << "]";
    }
    for (int j = 0; j < response.services_size(); ++j) {
        const std::string& s = response.services(j);
        LOG(INFO) << "UpdateServiceInfo(): add service [" << s << "]";
        const ::google::protobuf::ServiceDescriptor* sd = _desc_pool->FindServiceByName(s);
        if (sd == NULL) {
            LOG(ERROR) << "UpdateServiceInfo(): find service descriptor failed: " << s;
            delete _desc_pool;
            _desc_pool = NULL;
            return false;
        }
...
bool HttpAgent::ListService(std::map<std::string, ::google::protobuf::ServiceDescriptorProto>* service_map)
{
    ...
    if (_desc_pool == NULL) {
        LOG(ERROR) << "ListService(): descriptor pool not exist";
        return false;
    }
...
bool HttpAgent::GetDescriptor(const std::string& name, ProtobufType* type, std::string* desc)
{
    ReadLocker _(&_desc_lock);
    if (_desc_pool == NULL) {
        LOG(ERROR) << "GetDescriptor(): descriptor pool not exist";
        return false;
    }
    ...
    LOG(ERROR) << "GetDescriptor(): type not found: " << name;
    return false;
}
                           ...
                           ::google::protobuf::Closure* done)
{
    ...
    CHECK(sofa_controller != NULL); // should pass sofa::pbrpc::RpcController to CallMethod
    RpcControllerImplPtr cntl = sofa_controller->impl();

    if (_rpc_channel == NULL) {
        LOG(ERROR) << "CallMethod(): server not avaliable";
    ...
    if (_desc_pool == NULL && !UpdateServiceInfo()) {
        LOG(ERROR) << "CallMethod(): fetch service descriptor from server failed";
    ...
    if (!ParseMethodFullName(method_full_name, &service_full_name, &method_name)) {
        LOG(ERROR) << "CallMethod(): invalid method name: " << method_full_name;
    ...
    if (find ==  _svc_desc_map.end()) {
        LOG(ERROR) << "CallMethod(): service not found: " << service_full_name;
    ...
    if (md == NULL) {
        LOG(ERROR) << "CallMethod(): method not found: " << method_full_name;
    ...
    if (!::google::protobuf::TextFormat::ParseFromString(*request, request_message)) {
        ...
        LOG(ERROR) << "CallMethod(): parse request message failed";
    ...
    if (done == NULL) {
        ...
        if (!cntl->Failed()) {
            if (!::google::protobuf::TextFormat::PrintToString(*response_message, response)) {
                LOG(ERROR) << "CallMethod(): print response message failed";
